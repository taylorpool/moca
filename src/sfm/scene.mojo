...
struct SceneGraph:
    ...
    fn get_first_pair(inout self, inout pair: Tuple[Int, Int]) -> Tensor[DType.int64]:
        ...
        return truncate(mc.get_row(self.indices_pair, pair_id))

    fn get_ready_pair(inout self, inout active_poses: IntSet) -> Tensor[DType.int64]:
        ...
        for i in range(self.id_pairs.size()):
            ...
            if active_poses.contains(k.get[0, Int]()) and active_poses.contains(
                ...
                return truncate(mc.get_row(self.indices_pair, id_pair))

        return Tensor[DType.int64](0)

    fn get_next_pair(inout self, inout active_poses: IntSet) -> Tensor[DType.int64]:
        ...
        # Get the next image with the most landmarks
        var pair_max = 0
        var pair_id = 0
        for i in range(self.id_pairs.size()):
            var k = self.id_pairs.keys[i]
            if active_poses.contains(k.get[0, Int]()) or active_poses.contains(
                ...
                var id_pair = self.id_pairs.pop(k).__int__()
                var num_fc = self.lms_per_pair[id_pair].__int__()
                if num_fc > pair_max:
                    pair_max = num_fc
                    pair_id = id_pair
